...
<main role="main" class="responsive my-hm">
	...
	<script src="/2nd Project/04.21/project/Public/Backstage/js/distpicker.data.js"></script>
	<script src="/2nd Project/04.21/project/Public/Backstage/js/distpicker.js"></script>
	<script src="/2nd Project/04.21/project/Public/Backstage/js/main.js"></script>
	...
	<div class="wrapper">    
		<nav class="breadcrumbs">
			<ul>
				...
				<li class="">
					...
					<a href="#"  onclick="return false;"> 您的账户</a>
				</li>
				<li class="active">
		...
		<nav class="section navigation menu " role="navigation">
			<ul role="menu">
				<li class="item" role="presentation" tabindex="0">
					<a href="/2nd Project/04.21/project/index.php/Home/User/person" role="menuitem">概览</a>				
				</li>
		...
		<div class="layout layout-eight" >
			<div class="hidden modal-content" id="address-validation-popup">
						...
						<div class="address-validation-suggestions" ng-show="verifiedAddresses.length>0">
							...
							<div class="address-validation-address-list " ng-class="{'scrollable' : verifiedAddresses.length>4}" >
								<div class="address-validation-address" ng-repeat="address in verifiedAddresses">
									<input type="radio" name="validation-address" value="{<?php echo ($index); ?>}" id="validated-address-{<?php echo ($index); ?>}">
	...
	<header  id="header-pick" class="page-header">
		<h1>地址簿</h1>
		<p>您可在结帐页添加新地址。</p><br />
	</header>
		<?php if(empty($list) != true ): if(is_array($list)): foreach($list as $k=>$v): ?><div class="form-group"  >		
				<div ng-show="visibility.delivery2">
					<div class="default-fieldset" >
						...
						<input type="radio" value="<?php echo ($v['id']); ?>" data-uid = "<?php echo ($v['uid']); ?>" id="default_02" name="use-as-default" class="radio-input" 
					...
					<form  id="delivery2AddressForm" name="delivery2AddressForm" class="form-section new-form delivery2Form" >
					    <fieldset class="form-part">
							...
							<div class="inputwrapper" >
								...
								<input 
									...
									value="<?php echo ($v['recname']); ?>"	
									
									/>

								<span class="static static-recname" style="color:#555"> <?php echo ($v[recname]); ?></span>
							...
							<div class="inputwrapper" >
								<label class="label" for="checkout.line1"> 地址栏 </label>
									...
									autocomplete="given-name"
									value="<?php echo ($v['addr']); ?>"	
									/>
								<span class="static static-addr" style="color:#555"> <?php echo ($v[addr]); ?> </span>
							...
							<div class="inputwrapper">
								...
								<input 
									...
									autocomplete="given-name"
									value="<?php echo ($v['phone']); ?>"
									oninput="if(value.length>11)value=value.slice(0,11)" 	
									/>
								
								<span class="static static-phone" style="color:#555"><?php echo ($v[phone]); ?></span>				
							</div>
							...
							<div class="inputwrapper">
								...
								<input 
									...
									autocomplete="given-name"
									value="<?php echo ($v['zip']); ?>"	
									oninput="if(value.length>6)value=value.slice(0,6)" 
									/>
								
								<span class="static static-zip" style="color:#555" > <?php echo ($v[zip]); ?> </span>
						...
						<div class="button-group">
							...
							<a class="secondary button del-button" data-del="<?php echo ($v['id']); ?>" href="#remove-address" > 删除地址</a> 
							<button class="secondary button hideInfo edit-cancel" type="button" > 取消</button> 
							<button class="button save-form hideInfo save-addr " data-id="<?php echo ($v['id']); ?>" type="button" > 保存信息</button>
						</div>
					</form>
				</div>
			</div><?php endforeach; endif; endif; ?>

			<div class="form-group"  id="write-form" data-toggle="distpicker">		
				<div ng-show="visibility.delivery2">
	...
	<script>
		...
		$('#saveInfo').click(function () {
			...
			$.post('/2nd Project/04.21/project/index.php/Home/User/addAddress', {
				...
				if (data == 1){
					...
					var dass = "<div class='form-group' >";

					 dass +=	"<div ng-show='visibility.delivery2'><div class='default-fieldset' ><label for='default_00' class='preceding-label'>默认</label><input type='radio' id='default_02' name='use-as-default' class='radio-input' ></div><form  id='delivery2AddressForm' name='delivery2AddressForm' class='form-section new-form delivery2Form' ><fieldset class='form-part'><legend class='heading'> 邮递地址:0</legend><div class='inputwrapper' ><label class='label' for='checkout.lastName'> 姓名 </label><span class='static' style='color:#555'>"+$('#delivery2-lastName').val()+$('#delivery2-firstName').val()+"</span></div><div class='inputwrapper' ><label class='label' for='checkout.line1'> 地址栏 </label><span class='static'  style='color:#555'>"+$('#province').val()+$('#delivery2-town').val()+$('#delivery2-district').val()+$('#delivery2-line1').val()+"</span></div>";

					 dass += "<div class='inputwrapper'><label class='label' for='checkout.line2'>电话 </label><span class='static' style='color:#555'>"+$('#delivery-phone').val()+"</span></div>";

					 dass += "<div class='inputwrapper'><label class='label' for='checkout.postalCode'> 邮政编码 </label><span class='static static-zip' style='color:#555'>"+$('#delivery2-postalCode').val()+"</span></div>";

					 dass += "<div class='inputwrapper' ><label class='label' for='address-book.country'> 国家</label><span style='color:#555'> 中国 </span></div></fieldset>	<div class='button-group'><button class='button' type='button' >修改地址</button><a class='secondary button'href='#remove-address'> 删除地址</a></div>";



				$('#header-pick').after(dass);
        ...
        $('.edit-cancel').click( function () {
			...
			$(this).siblings('.save-addr').next('span').remove();
        ...
        $('.del-button').click(function () {
        	...
        	$.post('/2nd Project/04.21/project/index.php/Home/User/delAddress', { 	id:$(this).attr("data-del") }, function (data) {
        ...
        $('.radio-input').click( function () {
        	...
        	$.post('/2nd Project/04.21/project/index.php/Home/User/changeAddrStatus', {id: that.val(), uid:that.attr('data-uid')}, function (data) {
        ...
        $('.save-addr').click( function () {
        	...
        	if (them.find('.recname').val() == '') {

				that.next('span').remove();

				that.after("<span style='color:red'>请填写姓名!");

				return false;
			}
			if (them.find('.addr').val() == '') {

				that.next('span').remove();

				that.after("<span style='color:red'>请填写地址!");

				return false;
			}

			var	res = RegCheck(them.find('.phone').val(),/^1(3|4|5|7|8)\d{9}$/);
			...
			var	res = RegCheck(them.find('.zip').val(),/^[1-9]\d{5}$/);
        	...
        	$.post('/2nd Project/04.21/project/index.php/Home/User/editAddress', {
        		...
        		if (data == 1){
        			...
        			that.prev().hide();
        			that.prev().prev().show();
        			that.prev().prev().prev().show();
        			that.parent().prev().find('.static').show();
        			that.parent().prev().find('.text-input').hide();
        			them.find('.static-recname').html(them.find('.recname').val());
        			them.find('.static-addr').html(them.find('.addr').val());
        			them.find('.static-phone').html(them.find('.phone').val());
        			them.find('.static-zip').html(them.find('.zip').val());
