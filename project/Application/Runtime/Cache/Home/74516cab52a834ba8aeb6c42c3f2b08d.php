							...
							<div id="cartPage-body-empty" class="box">
								<div class="box-content shopping-bag-empty">
									...
									<div class="ng-scope">
										...
										<br><br>
										<a href="/last/LastMission/project/index.php/Home/User/signIn">登录</a>
										<?php else: ?>
			...
			<div class="product-detail-list-item-actions">
				...
				<div class="select-primary product-detail-list-item-quantity">
    				<select name="product-detail-list-item-quantity-options-4" id="shopping-bag-quantity-options-4" data-validation="false" ng-change="updateQuantity(entry,entry.quantity)" ng-model="entry.quantity" ng-options="option for option in range(entry.maxQuantity)" class="ng-pristine ng-untouched ng-valid num-sel"  aid="<?php echo ($ov["aid"]); ?>" gid="<?php echo ($ov["gid"]); ?>">
    					<?php if($ov['stock'] >= 10): $__FOR_START_5464__=1;$__FOR_END_5464__=11;for($i=$__FOR_START_5464__;$i < $__FOR_END_5464__;$i+=1){ ?><option value="<?php echo ($i); ?>" 
    							<?php if($i == $ov['gnum']): ?>selected<?php endif; ?> label="<?php echo ($i); ?>"><?php echo ($i); ?></option><?php } ?>
    					<?php else: ?>
    						<?php $__FOR_START_5809__=1;$__FOR_END_5809__=$ov['stock'] + 1;for($i=$__FOR_START_5809__;$i < $__FOR_END_5809__;$i+=1){ ?><option value="<?php echo ($i); ?>" 
...
<script>
	...
	$(document).on('click', '.rm-btn', function () {
		...
		$.get(
			'/last/LastMission/project/index.php/Home/Cart/delFromCart',
			{
				aid:aid,
				gid:gid
			},
	...
	$(document).on('click', '.fav-btn', function () {
		...
		$.get(
			'/last/LastMission/project/index.php/Home/Cart/cartToFav',
			{
				aid:aid,
				gid:gid
			},
	...
	$(document).on('change', '.num-sel', function () {
		...
		$.get(
			'/last/LastMission/project/index.php/Home/Cart/changeNum',
			{
				gnum:gnum,
				aid:aid,
				gid:gid
	...
	$('#goToQuery').click(function () {
		location.href = '/last/LastMission/project/index.php/Home/order/checkout';
	});
</script>
        ...
        <script>
            ...
            $('.lists').on('mouseover', function() {
                ...
                var url = '/last/LastMission/project/index.php/Home/Cart/base';
