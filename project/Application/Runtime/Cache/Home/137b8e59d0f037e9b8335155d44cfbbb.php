...
<head>
       ...
       <title>H&M.Update Password</title>
</head>
	 ...
	 <div class="layout layout-eight">
    	...
    	<script>
    		...
    		$('#saveBtn').click( function () {
    			...
    			if ($('#txt-password').val().length < 6 || $('#txt-password').val().length > 12) {

    				$(this).next('span').remove();

                   $(this).after('<span style="color:red";>*密码长度不符合!</span>');
    			...
    			if ($('#txt-password').val() != $('#txt-password-confirm').val()){
                   ...
                   $(this).after('<span style="color:red";>*密码与确认密码不一样!</span>');

                   return false;
                }


    			var res = RegCheck($('#txt-password').val(), /^\w{6,12}$/);

                if (res === null) {

                   $(this).next('span').remove();

                   $(this).after('<span style="color:red";>*密码由数字字母下划线组成!</span>');
    			...
    			$.post('/xxoooo/Project/project/index.php/Home/User/updatePassword', {
