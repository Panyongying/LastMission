...
<fieldset>	
	...
	<p class="text">您可在<a href="/2nd Project/04.21/project/index.php/Home/User/account"> 我的H&amp;M </a>中编辑地址</p>
	<h3>使用现有地址</h3>
	...
	<button class="button secondary newAddr-btn" style="margin-top: 10px;">新建地址</button>
...
<span ng-if="entry.totalPrice !== 0" class="ng-binding ng-scope">
	...
	<tbody>
		<tr>
			...
			<td><span id="newPriceSubtotal" class="ng-binding">¥<?php echo ($data["totalPrice"]); ?></span></td>
		</tr>		
	</tbody>
	<tfoot>
		<tr>
...
<script>
	...
	$('#ad-btn').click(function () {
		...
		if (flag) {
			...
			$.post(
				'/2nd Project/04.21/project/index.php/Home/order/addAddress',
				{
					recname:recname.val(),
				...
				function (data) {
					if (data == 2) {
						...
						that.siblings().remove();
						$('#zzc').css('display', 'none');
						$('.choose-delivery-address').children().remove();
	...
	$('#placeorderbutton').click(function () {
		if ($('input[name="addrid"]:checked').val() == undefined) {
			$(this).prev().remove();
		...
		$.post(
			'/2nd Project/04.21/project/index.php/Home/Order/queryOrder',
			{
				addrid:$('input[name="addrid"]').val()
			},
			function (data) {
				if (data == 2) {
					$('#desktopPaymentConfirmationLoader').children().children().children().remove();
					$('#desktopPaymentConfirmationLoader').children().children().append('<p class="text"> 系统繁忙，请刷新页面重新提交</p>');
				} else {
					location.href = '/2nd Project/04.21/project/index.php/Home/Order/queryOrder/oid/' + data;
				}
			},
			'json'
		);
        ...
        <script>
            ...
            $('.lists').on('mouseover', function() {
                ...
                var url = '/2nd Project/04.21/project/index.php/Home/Order/base';
