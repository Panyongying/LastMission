...
<div class="admin-biaogelist">
    ...
    <div class="fbneirong">
      <form  class="am-form am-g" action="/last/LastMission/project/index.php/HMadmin/Goods/stockEdit" method="post">
        库存：<input type="text" name="goodsNum" value="<?php echo ($stockOne["goodsnum"]); ?>">
        颜色：<select name="aid[]">
        ...
        <input type="hidden" name="id" value="<?php echo ($stockOne["id"]); ?>">
      </form>
    </div>

    <script>

      var value = <?php echo ($stockOne['aid'][0]); ?>;
      var values = <?php echo ($stockOne['aid'][1]); ?>;
      var v = $('.attrColor');
      var vs = $('.attrSize');
      ...
      for (var i=0; i<v.length; i++) {
        ...
        if ( value == v[i].value ) {
          ...
          v[i].setAttribute('selected', true);
        }
      }

      for (var i=0; i<vs.length; i++) {

        if ( values == vs[i].value ) {

          vs[i].setAttribute('selected', true);
        }
      }
